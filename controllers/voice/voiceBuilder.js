...
const syncItemToUsers = async (sectionKey, itemId, isActive) => {
  if (isActive) {
    await User.updateMany(
      {},
      { $addToSet: { [`permissions.voiceBuilder.${sectionKey}.allowedItems`]: itemId } }
    );
  } else {
    await User.updateMany(
      {},
      { $pull: { [`permissions.voiceBuilder.${sectionKey}.allowedItems`]: itemId } }
    );
  }
};
...
export const toggleTypeSection = async (req, res) => {
  try {
    ...
    await User.updateMany(
      {},
      { $set: { "permissions.voiceBuilder.types.isActive": doc.types.isActive } }
    );
    res.status(200).json({ success: true, data: doc.types });
  } catch (err) { res.status(500).json({ success: false, message: err.message }); }
};
...
export const patchTone = async (req, res) => {
  try {
    ...
    await doc.save();
    if (typeof req.body.isActive === "boolean") {
      await syncItemToUsers("tones", req.params.itemId, req.body.isActive);
    }
    res.status(200).json({ success: true, data: doc.tones });
  } catch (err) { res.status(500).json({ success: false, message: err.message }); }
};
...
export const toggleToneSection = async (req, res) => {
  try {
    ...
    await User.updateMany(
      {},
      { $set: { "permissions.voiceBuilder.tones.isActive": doc.tones.isActive } }
    );
    res.status(200).json({ success: true, data: doc.tones });
  } catch (err) { res.status(500).json({ success: false, message: err.message }); }
};
...
export const patchPacing = async (req, res) => {
  try {
    ...
    await doc.save();
    if (typeof req.body.isActive === "boolean") {
      await syncItemToUsers("pacing", req.params.itemId, req.body.isActive);
    }
    res.status(200).json({ success: true, data: doc.pacing });
  } catch (err) { res.status(500).json({ success: false, message: err.message }); }
};
...
export const togglePacingSection = async (req, res) => {
  try {
    ...
    await User.updateMany(
      {},
      { $set: { "permissions.voiceBuilder.pacing.isActive": doc.pacing.isActive } }
    );
    res.status(200).json({ success: true, data: doc.pacing });
  } catch (err) { res.status(500).json({ success: false, message: err.message }); }
};
...
export const togglePresetSection = async (req, res) => {
  try {
    ...
    await User.updateMany(
      {},
      { $set: { "permissions.voiceBuilder.presets.isActive": doc.presets.isActive } }
    );
    res.status(200).json({ success: true, data: doc.presets });
  } catch (err) { res.status(500).json({ success: false, message: err.message }); }
};